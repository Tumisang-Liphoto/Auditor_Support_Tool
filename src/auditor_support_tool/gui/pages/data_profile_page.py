...
class DataProfilePage(QWidget):
    ...
    def _build_summary_card(self) -> QFrame:
        ...
        self._dataset_selector_hint = QLabel(
            "Only datasets included and confirmed in Data "
            "Sources are available here."
        )
        self._dataset_selector_hint.setObjectName(
            "fieldHint"
        )
        ...
        self._summary_status = QLabel(
            "Confirm datasets in Data Sources before "
    ...
    def _build_columns_card(self) -> QFrame:
        ...
        self._table_status = QLabel(
            "No confirmed dataset is available."
        )
    ...
    def _connect_signals(self) -> None:
        self._dataset_selector.currentIndexChanged.connect(
            self._dataset_selection_changed
        )

        self._workspace_state.source_changed.connect(
            self._refresh_page
        )
        self._workspace_state.population_loaded.connect(
            self._refresh_page
        )
        self._workspace_state.profile_created.connect(
            self._refresh_page
        )
    ...
    def _refresh_dataset_selector(self) -> None:
        selected_datasets = (
            self._workspace_state.selected_datasets
        )
        active_dataset_id = (
        ...
        try:
            ...
            self._dataset_selector.setEnabled(
                self._dataset_selector.count() > 0
            )

            active_index = (
                self._dataset_selector.findData(
                    active_dataset_id
                )
            )

            if active_index >= 0:
                self._dataset_selector.setCurrentIndex(
                    active_index
                )
    ...
    def _dataset_selection_changed(
        ...
        if (
            self._updating_dataset_selector
            or index < 0
        ):
            return

        dataset_id = self._dataset_selector.itemData(
            index
        )
    ...
    def _refresh_page(self) -> None:
        ...
        active_dataset = (
            self._workspace_state.active_dataset
        )

        if (
            active_dataset is None
            or active_dataset.status
            != PreparationStatus.CONFIRMED
        ):
            self._clear_summary()
            ...
            self._set_summary_status(
                (
                    "Confirm at least one dataset in Data "
                    "Sources before reviewing its profile."
                ),
                "neutral",
            )
        ...
        if profile is None:
            self._source_file_value.setText(
                table.source_path.name
            )
            self._worksheet_value.setText(
                active_dataset.original_worksheet_name
            )
            self._dataset_name_value.setText(
                active_dataset.confirmed_display_name
            )
            self._dataset_type_value.setText(
                self._dataset_type_label(
                    active_dataset
                    .confirmed_dataset_type.value
                )
            )
            ...
            self._table_status.setText(
                (
                    "The dataset profile could not be "
                    "displayed."
                )
            )
            return
    ...
    def _display_profile(
        ...
        self._dataset_type_value.setText(
            self._dataset_type_label(dataset_type)
        )
        self._records_value.setText(
        ...
        self._blank_cells_value.setText(
            f"{profile.blank_cell_count:,}"
        )
        self._columns_with_blanks_value.setText(
            f"{profile.columns_with_blanks:,}"
        )

        self._columns_table.setSortingEnabled(False)
        self._columns_table.setRowCount(
            len(profile.columns)
        )

        for row_number, column in enumerate(
            profile.columns
        ):
        ...
        self._set_summary_status(
            (
                "The selected dataset profile is "
                "available."
            ),
            "success",
        )
    ...
    def _set_summary_status(
        ...
        self._summary_status.style().unpolish(
            self._summary_status
        )
        self._summary_status.style().polish(
            self._summary_status
        )
    ...
    def _format_samples(
        ...
        return " | ".join(
            DataProfilePage._format_value(value)
            for value in values
        )
